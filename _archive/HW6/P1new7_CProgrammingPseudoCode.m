...
% ADC INPUT RELATED
    ...
    kill_freq = unique(round(abs(atan(imag(Zdig_sort) ...
        ./real(Zdig_sort))),10))./T; % kill freq in Hz
u = @(n) (n>0) + 0.5.*(n==0);
N = 101;

for m = 1:length(kill_freq)
    X = @(t) 2048*sin(kill_freq(m)*t).*(mod(t,1)==0);
    Y = [];
    filter_select = 1;
    Gain = real(G);
    ...
    figure(); subplot(211); stem((1:N).*T,X(1:N),'k'); xlabel('seconds'); ylabel('x[n]');
    subplot(212); stem((1:N).*T,Y,'k'); xlabel('seconds'); ylabel('y[n]');
